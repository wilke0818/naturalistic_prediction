...
def evaluate_with_held_out_subject(
    ...
    for subject in sub_datasets:
        ...
        for batch in trained_loader:
            ...
            for i in range(targets.shape[0]):
                    ...
                    subject_fmris[subject][ep_id] = fmri[i][:lengths[i],:][5:-5]
    
    subject_attentions[held_out_subject] = {}
    subject_fmris[held_out_subject] = {}
    subject_result = {}
    for batch in test_out_dist_loader:
            ...
            for i in range(targets.shape[0]):
                    ...
                    subject_fmris[held_out_subject][ep_id] = fmri[i][:lengths[i],:][5:-5]

    subject_maps = {}
    for ep_id in subject_attentions[held_out_subject]:
        for subj in subject_attentions:
            if subj not in subject_maps:
                subject_maps[subj] = []
            #print(subject_fmris[subj][ep_id].shape)
            #print(np.array(subject_attentions[subj][ep_id]).shape)
            glm_labels, glm_results = run_glm(subject_fmris[subj][ep_id].numpy(),np.array(subject_attentions[subj][ep_id]))
            #print(glm_labels)
            contrast_result = glm.compute_contrast(glm_labels,glm_results,np.array([1]))
            result_dir = f"{model_dir}/attention_glm_contrasts/{held_out_subject}"
            os.makedirs(result_dir, exist_ok=True)
            _, p_vals_corrected = fdrcorrection(contrast_result.p_value())
            subject_maps[subj].append(p_vals_corrected)
            np.savez(f"{result_dir}/{subj}_{ep_id}_contast_results.npz",**{'p_values':contrast_result.p_value(),'p_values_corrected': p_vals_corrected,'t_stats':contrast_result.stat()})
    
    for subj in subject_maps:
        for i,u_thresh in enumerate([1,math.ceil(.25*len(subject_maps[subj])),math.ceil(.5*len(subject_maps[subj])),math.ceil(.75*len(subject_maps[subj])),len(subject_maps[subj])]):
            pooled_subject_map = pool_dependent_p(np.array(subject_maps[subj]),u_thresh)
            binary_subject_map = fdr_pooled_procedure(pooled_subject_map)
            np.savez(f"{result_dir}/{subj}_pooled-{i}_contast_results.npz",**{'p_values_corrected': pooled_subject_map,'binary_mask':binary_subject_map})
            #print(contrast_result.p_value(),contrast_result.stat(),contrast_result.effect_size())
            #print([glm_results[label] for label in glm_labels])
            #ind_glm = []
          #  for i,label in enumerate(glm_labels):
          #      num_params_in_current_model = glm_results[label].theta.shape[1]
          #      contrast_array = np.zeros(num_params_in_current_model)
          #      contrast_array[i]=1
          #      ind_glm.append(glm_results[label].Tcontrast(contrast_array))
          #      #if i == 0:
          #      #    ind_glm.append(glm_results[label].Tcontrast(np.array([1.0])))
          #      #else:
          #      #    ind_glm.append(glm_results[label].Tcontrast(i))
          #  
          #  print(subj,ind_glm)
        #if len(set([subject_attentions[subj][ep_id].shape[0] for subj in subject_attentions]))!=1:continue
        #group_attn = np.array([subject_attentions[subj][ep_id].numpy() for subj in subject_attentions])
        #print(group_attn.T.shape)
        #group_glm = run_glm(subject_fmris[held_out_subject][ep_id],group_attn.T)
        #print('group glm',group_glm)
