...
def setup_logging(base_dir, task, sub_id=None, parcellation='tian'):
    ...
    if sub_id:
        ...
        held_out = 'all'
    log_file = os.path.join(log_dir, f'{parcellation}_{task}_{held_out}_binary_{timestamp}.log')
...
def get_subject_localizer_data(sub_id, task, localizer_output_dir, parcellation='tian',binary=False):
    # sub-01_wm_stats.npz
...
def train_with_early_stopping(model, train_loader, val_loader, optimizer, device, loss_fn=nn.BCEWithLogitsLoss(), num_epochs=20, patience=5, delta=0,checkpoint_dir=None, checkpoint_prefix="model"):
        ...
        with torch.no_grad():
            ...
            for batch in test_out_dist_loader:
                ...
                for i in range(targets.shape[0]):
                    loss = metrics.roc_auc_score(targets[i],outputs[i])#np.corrcoef(outputs[i], targets[i])
...
def main():
    ...
    for subject_id in subject_ids:
        ...
        if subject_id == args.sub_id:
            localizer_data = get_subject_localizer_data(subject_id, args.task, localizer_dir, args.parcellation,binary=True)
