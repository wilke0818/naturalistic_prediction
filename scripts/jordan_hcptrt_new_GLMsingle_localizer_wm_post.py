...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run=None,parcellation=None):
    ...
    parcellate = parcellation if parcellation else "unparcellated"
    if parcellation:
        # Prepare data for statistical comparison
        two_back = np.concatenate((stim_arrays[unique_stims[4]],stim_arrays[unique_stims[5]],stim_arrays[unique_stims[6]],stim_arrays[unique_stims[7]]),axis=1)
        zero_back = np.concatenate((stim_arrays[unique_stims[0]],stim_arrays[unique_stims[1]],stim_arrays[unique_stims[2]],stim_arrays[unique_stims[3]]),axis=1)

        # Statistical testing
    
        # Only compute statistics for signal vertices
        signal_indices = np.where(signal_mask)[0]
        two_back_parcels = voxel_parcellation(two_back, signal_indices,parcellation)
        zero_back_parcels = voxel_parcellation(zero_back,signal_indices,parcellation)

        average_betas_stimuli = np.zeros((two_back_parcels.shape[0],two_back.shape[1]))
        average_betas_contrast = np.zeros((two_back_parcels.shape[0],two_back.shape[1]))

        t_stats_full = np.zeros(two_back_parcels.shape[0])  # Initialize with full brain size
        p_values_full = np.ones(two_back_parcels.shape[0])-.00000001

        logging.info(f"parcel shape: {two_back_parcels.shape} and {zero_back_parcels.shape}")

        count_warnings =0 
        for signal_idx in range(two_back_parcels.shape[0]):
            t_stat, p_val = stats.ttest_ind(two_back_parcels[signal_idx, :], zero_back_parcels[signal_idx, :],alternative='greater')
            if math.isnan(t_stat) or math.isnan(p_val):
                t_stats_full[signal_idx] = 0
                p_values_full[signal_idx] = 1-.00000001
                count_warnings+=1
                warnings.warn(f"t_stat is {t_stat} and p_val is {p_val} at {signal_idx}, {two_back_parcels[signal_idx, :]} > {zero_back_parcels[signal_idx, :]}",RuntimeWarning)
            else:
                t_stats_full[signal_idx] = t_stat
                p_values_full[signal_idx] = p_val
            average_betas_stimuli[signal_idx] = two_back_parcels[signal_idx, :]
            average_betas_contrast[signal_idx] = zero_back_parcels[signal_idx, :]

        warnings.warn(f"Saw {count_warnings} warnings", RuntimeWarning)
    
        binary_mask = np.zeros(two_back_parcels.shape[0])
        rejected, corrected_p = fdrcorrection(p_values_full, alpha=0.05)
        binary_mask[np.where((rejected) & (t_stats_full > 0))[0]] = 1

        average_betas_two = np.average(average_betas_stimuli,axis=1)
        average_betas_zero = np.average(average_betas_contrast,axis=1)
        ...
        stats_dict = {
            ...
            'betas_stimuli': average_betas_two,
            'betas_contrast': average_betas_zero
        }
        stats_file = os.path.join(output_dir, f"{sub_id}_{task}_stats_{parcellate}.npz")
        np.savez(stats_file, **stats_dict)
        logging.info(f"Saved statistics to: {stats_file}")
    else:
        # Prepare data for statistical comparison
        two_back = np.concatenate((signal_arrays[unique_stims[4]],signal_arrays[unique_stims[5]],signal_arrays[unique_stims[6]],signal_arrays[unique_stims[7]]),axis=1)
        zero_back = np.concatenate((signal_arrays[unique_stims[0]],signal_arrays[unique_stims[1]],signal_arrays[unique_stims[2]],signal_arrays[unique_stims[3]]),axis=1)
        ...
        average_betas_stimuli = np.zeros((91282,two_back.shape[1]))
        average_betas_contrast = np.zeros((91282,two_back.shape[1]))

        logging.info(f"parcel shape: {two_back.shape} and {zero_back.shape}")
        count_warnings = 0
        for i,signal_idx in enumerate(signal_indices):
            t_stat, p_val = stats.ttest_ind(two_back[i, :], zero_back[i, :],alternative='greater')
            if math.isnan(t_stat) or math.isnan(p_val):
                t_stats_full[signal_idx] = 0
                p_values_full[signal_idx] = 1-.00000001
                warnings.warn(f"t_stat is {t_stat} and p_val is {p_val} at {signal_idx}, {two_back[i, :]} > {zero_back[i, :]}",RuntimeWarning)
                count_warnings += 1
            else:
                t_stats_full[signal_idx] = t_stat
                p_values_full[signal_idx] = p_val
            average_betas_stimuli[signal_idx] = two_back[i, :]
            average_betas_contrast[signal_idx] = zero_back[i, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--parcellation", help="parcellate?",type=str,default="")
    ...
    if bool(args.runs):
        ...
        for run in runs:
            ...
            stim_files = open(os.path.join(f"{scratch_dir}/hcptrt/output/GLMsingle/", f"{args.sub_id}_{args.task}_unique_predictors.txt"), "r").readlines()
            output_dir = f"{scratch_dir}/hcptrt/output/GLMsingle/mask/{parcellation}/{args.sub_id}/{args.task}/{run}/"
