#!/bin/bash
#SBATCH --job-name=GLMsingle_localizer_face_ttest
...
if [ -n "$1" ]; then
    python jordan_hcptrt_new_GLMsingle_localizer_face_post.py "${TASK_ID}" --parcellation "$1"
else
    python jordan_hcptrt_new_GLMsingle_localizer_face_post.py "${TASK_ID}"
fi
