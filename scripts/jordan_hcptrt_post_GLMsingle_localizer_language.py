...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run):
    ...
    language_story = stim_arrays[unique_stims[1]]
    math_story = stim_arrays[unique_stims[0]]
    ...
    language_parcels = voxel_parcellation(language_story, signal_indices)
    math_parcels = voxel_parcellation(math_story, signal_indices)
    for signal_idx in range(language_parcels.shape[0]):
        t_stat, p_val = stats.ttest_ind(language_parcels[signal_idx, :], math_parcels[signal_idx, :])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", help="Task", type=str, default="language")
