...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run):
    ...
    face = stim_arrays[unique_stims[0]]
    shape = stim_arrays[unique_stims[1]]
    ...
    face_parcel = voxel_parcellation(face, signal_indices)
    shape_parcel = voxel_parcellation(shape, signal_indices)
    for parcel_idx in range(face_parcel.shape[0]):
        t_stat, p_val = stats.ttest_ind(face_parcel[parcel_idx,:], shape_parcel[parcel_idx, :])
        #if np.average(face[i,:])==np.average(shape[i,:]):
         #   print('average;test condition',face[i, :])
         #   print('avg;control condition', shape[i, :])
        if (math.isnan(t_stat) or math.isnan(p_val)):
            t_stats_full[parcel_idx] = 0
            p_values_full[parcel_idx] = 1
            ...
            t_stats_full[parcel_idx] = t_stat
            p_values_full[parcel_idx] = p_val
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", help="Task", type=str, default="emotion")
