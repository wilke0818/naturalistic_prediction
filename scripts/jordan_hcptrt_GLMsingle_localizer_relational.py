...
def load_event_and_json_files(event_files: List[str], json_files: List[str], brain_files: List[str], tr: float) -> List[np.ndarray]:
    ...
    for i, (event_file, json_file) in enumerate(zip(event_files, json_files)):
        try:
            ...
            df = df.dropna(subset=["stim_file"]) # we can drop countdown in this way
...
#                    f.write(f"{trial}\n")
        ...
        print(run_design_matrices)

        # Add validation checks before GLMsingle fit
        print(f"Design matrices shape: {[dm.shape for dm in run_design_matrices]}")
        print(f"Brain data shape: {[bd.shape for bd in run_brain_data]}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="relational")  # Optional with default
    ...
    if args.sub_id == 'sub-06':
        index_to_remove = event_files.index(Path('/orcd/data/satra/002/datasets/cneuromod.processed/fmriprep/hcptrt/sourcedata/hcptrt/sub-06/ses-002/func/sub-06_ses-002_task-relational_run-01_events.tsv'))
        event_file =  event_files.pop(index_to_remove)
        #event_files.remove('/orcd/data/satra/002/datasets/cneuromod.processed/fmriprep/hcptrt/sourcedata/hcptrt/sub-06/ses-002/func/sub-06_ses-002_task-relational_run-01_events.tsv')
        json_file = json_files.pop(index_to_remove)
        #json_files.remove('/orcd/data/satra/002/datasets/cneuromod.processed/fmriprep/hcptrt/sourcedata/hcptrt/sub-06/ses-002/func/sub-06_ses-002_task-relational_run-01_event_bold.json')
        brain_file = brain_files.pop(index_to_remove)
        with open(json_file, "r") as f:
            json_data = json.load(f)
        try:
            n_tr = len(json_data["time"]["samples"]["AcquisitionNumber"])
            ...
            logging.warning(f"AcquisitionNumber not found in {json_file}, using brain file shape instead")
            n_tr = nib.load(brain_file).shape[0]
        print(n_tr)
        df = pd.read_csv(event_file, sep="\t")
        df = df.dropna(subset=["stim_file"]) # we can drop countdown in this way
        df = df.dropna(subset=["onset"])
        df["onset_TR"] = df["onset"] / 1.49
        print(df["onset_TR"])
