...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run):
    ...
    control = stim_arrays[unique_stims[0]]
    relational = stim_arrays[unique_stims[1]]
    logging.info(f"Comparing {unique_stims[1]} vs {unique_stims[0]}")
    ...
    control_parcels = voxel_parcellation(control, signal_indices)
    relational_parcels = voxel_parcellation(relational, signal_indices)
    for signal_idx in range(relational_parcels.shape[0]):
#        print('relational',relational[i, :])
#        print('control',control[i, :])
        t_stat, p_val = stats.ttest_ind(relational_parcels[signal_idx, :], control_parcels[signal_idx, :])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", help="Task", type=str, default="relational")
