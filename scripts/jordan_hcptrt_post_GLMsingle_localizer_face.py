...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run):
    ...
    face = stim_arrays[unique_stims[1]]
    place = stim_arrays[unique_stims[2]]
    ...
    face_parcels = voxel_parcellation(face, signal_indices)
    place_parcels = voxel_parcellation(place,signal_indices)


    logging.info(f"parcel shape: {face_parcels.shape} and {place_parcels.shape}")

    for signal_idx in range(face_parcels.shape[0]):
        t_stat, p_val = stats.ttest_ind(face_parcels[signal_idx, :], place_parcels[signal_idx, :])
    ...
    #print(p_values_full)
    #print(corrected_p-p_values_full)
    binary_mask[np.where(rejected)[0]] = 1
