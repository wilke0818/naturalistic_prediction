#!/bin/bash
#SBATCH --job-name=GLMsingle_localizer_baseline
...
if [ -n "$1" ]; then
    python jordan_hcptrt_new_GLMsingle_localizer_baseline.py "${TASK_ID}" --parcellation "$1"
else
    python jordan_hcptrt_new_GLMsingle_localizer_baseline.py "${TASK_ID}"
fi
