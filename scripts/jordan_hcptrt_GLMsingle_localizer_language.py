...
def get_binary_df(df: pd.DataFrame, n_tr: int, sorted_unique_trial_types: List[str], stimdur: float, tr: float) -> pd.DataFrame:
    """
    ...
    end_buffer_tr = int(np.ceil(8 / tr))

    for row in df.itertuples():
        if pd.notna(row.trial_type) and pd.notna(row.duration):
            onset = int(row.onset_TR)
            trial_type = row.trial_type
            trial_duration = row.duration
            
            # Skip trials that start too close to the end
            if onset > (n_tr - end_buffer_tr):
                continue
            
            # Calculate number of segments needed
            n_segments = int(np.round(trial_duration / stimdur))
            # Calculate TR interval for each segment
            tr_interval = int(np.round(stimdur / tr))
            
            # Add markers for each segment
            for i in range(n_segments):
                segment_onset = onset + (i * tr_interval)
                # Check if this segment would be too close to the end
                if segment_onset >= (n_tr - end_buffer_tr):
                    break
                binary_df.at[segment_onset, trial_type] = 1
...
def load_event_and_json_files(event_files: List[str], json_files: List[str], brain_files: List[str], tr: float, stimdur: float) -> List[np.ndarray]:
    """
    ...
    for i, (event_file, json_file) in enumerate(zip(event_files, json_files)):
        try:
            ...
            df = df.dropna(subset=["stim_file","stim_file_question1"], how="all") # we can drop countdown in this way
            ...
            sorted_unique_trial_types = ["presentation_math", "question_math", "presentation_story", "question_story"]
            binary_df = get_binary_df(df, n_tr, sorted_unique_trial_types,stimdur,tr)
...
def main(sub_id: str, task: str, tr: float, n_jobs: int, event_files: List[str], json_files: List[str], brain_files: List[str], output_dir: str):
    ...
    design_matrices, trial_orders  = load_event_and_json_files(event_files, json_files, brain_files, tr,stimdur)
    ...
    for i in range(len(run_names+["all"])):
        ...
        if run=='all':
            ...
            midpoint = run_design_matrices.shape[0]//2
            run_design_matrices = [run_design_matrices[0:midpoint,:],run_design_matrices[midpoint:,:]]
            run_brain_data = [run_brain_data[:,0:midpoint],run_brain_data[:,midpoint:]]
