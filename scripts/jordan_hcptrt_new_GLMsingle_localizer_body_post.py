...
def main(sub_id, task, glm_results, designinfo, stim_files, output_dir, run=None,parcellation=None):
    ...
    if parcellation:
        ...
        body = np.concatenate((stim_arrays[unique_stims[0]],stim_arrays[unique_stims[4]]),axis=1)
        others = np.concatenate((stim_arrays[unique_stims[1]],stim_arrays[unique_stims[2]],stim_arrays[unique_stims[3]],stim_arrays[unique_stims[5]],stim_arrays[unique_stims[6]],stim_arrays[unique_stims[7]]),axis=1)

    
        # Only compute statistics for signal vertices
        signal_indices = np.where(signal_mask)[0]
        body_parcels = voxel_parcellation(body, signal_indices,parcellation)
        others_parcels = voxel_parcellation(others,signal_indices,parcellation)
        
        average_betas_stimuli = np.zeros((body_parcels.shape[0],body.shape[1]))
        average_betas_contrast = np.zeros((others_parcels.shape[0],others.shape[1]))
        t_stats_full = np.zeros(body_parcels.shape[0])  # Initialize with full brain size
        p_values_full = np.ones(body_parcels.shape[0])-.00000001

        logging.info(f"parcel shape: {body_parcels.shape} and {others_parcels.shape}")

        count_warnings = 0
        for signal_idx in range(body_parcels.shape[0]):
            t_stat, p_val = stats.ttest_ind(body_parcels[signal_idx, :], others_parcels[signal_idx, :],alternative='greater')
            if math.isnan(t_stat) or math.isnan(p_val):
                t_stats_full[signal_idx] = 0
                p_values_full[signal_idx] = 1-.00000001
                count_warnings += 1
                warnings.warn(f"t_stat is {t_stat} and p_val is {p_val} at {signal_idx}, {body_parcels[signal_idx, :]} > {others_parcels[signal_idx, :]}",RuntimeWarning)
            else:
                t_stats_full[signal_idx] = t_stat
                p_values_full[signal_idx] = p_val
            average_betas_stimuli[signal_idx] = body_parcels[signal_idx, :]
            average_betas_contrast[signal_idx] = others_parcels[signal_idx, :]
    
        binary_mask = np.zeros(body_parcels.shape[0])
        rejected, corrected_p = fdrcorrection(p_values_full, alpha=0.05)
        binary_mask[np.where((rejected) & (t_stats_full > 0))[0]] = 1

        average_betas_body = np.average(average_betas_stimuli,axis=1)
        ...
        stats_dict = {
            ...
            'betas_stimuli': average_betas_body,
        ...
        # Prepare data for statistical comparison
        body = np.concatenate((signal_arrays[unique_stims[0]],signal_arrays[unique_stims[4]]),axis=1)
        others = np.concatenate((signal_arrays[unique_stims[1]],signal_arrays[unique_stims[2]],signal_arrays[unique_stims[3]],signal_arrays[unique_stims[5]],signal_arrays[unique_stims[6]],signal_arrays[unique_stims[7]]),axis=1)
        ...
        average_betas_stimuli = np.zeros((91282,body.shape[1]))
        average_betas_contrast = np.zeros((91282,others.shape[1]))
        logging.info(f"parcel shape: {body.shape} and {others.shape}")

        count_warnings = 0
        for i,signal_idx in enumerate(signal_indices):
            t_stat, p_val = stats.ttest_ind(body[i, :], others[i, :],alternative='greater')
            if math.isnan(t_stat) or math.isnan(p_val):
                t_stats_full[signal_idx] = 0
                p_values_full[signal_idx] = 1-.00000001
                warnings.warn(f"t_stat is {t_stat} and p_val is {p_val} at {signal_idx}, {body[i, :]} > {others[i, :]}",RuntimeWarning)
                count_warnings += 1
            else:
                t_stats_full[signal_idx] = t_stat
                p_values_full[signal_idx] = p_val
            average_betas_stimuli[signal_idx] = body[i, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", help="Task", type=str, default="body")
